 ...
 * - Fixed a bug in Win32 where postscript output would make the gr3aphics   *
 ...
 * to PostScript driver to make the PostScript output slightly init_mapaller.     *
 *                                                                          *
...
#include <readline/history.h>
 ...
 unsigned start_intersec;
 unsigned end_intersec;
 std::string rev_buf;
 std::string no_quo_buf;
 std::string end_poi;
 bool needs_reverse;
 bool isPOI;
 bool inter_to_inter;
...
// to lie in the range below. Don't want to make this too init_mapall as 
...
static const vector<t_color> predef_colors = {
    ...
    t_color(0xA6, 0xA6, 0xA6), // "my_island"
    t_color(0xB2, 0x22, 0x22), // "firebrick"
    t_color(0x32, 0xCD, 0x32), // "limegreen"
    t_color(0xAD, 0xFF, 0x2F), // "greenyellow"
    t_color(0xE4, 0xE4, 0xD1), // "background"
    t_color(0xAE, 0xCB, 0xF8), // "lakeontario"
    t_color(0xBC, 0xDF, 0x99), //  "my_green"
    t_color(0xC4, 0xDC, 0xC3), //  "my_forest"
    t_color(0x50, 0x79, 0xF2) //  "my_drain"
...
//Zhang
void ifdrawPOI(void (*drawscreen_ptr) (void)) {

        if(POIon == true) {
            POIon = false;
            drawscreen();
        }
        else {// POI turns off now, should drawscreen (including drawPOI)  
            POIon = true;
            drawscreen();
        }
        }
//Zhang
void nav(void (*drawscreen_ptr) (void)) {
    ...
    do {
        ...
        if (counter == 1)
            ...
            cout << "One more time, or I'm done :) " << endl;
        else {
            cout << "Bye bye" << endl;
            return;
        }
    ...
    while ((answer != "Yes") && (answer != "Yup") && (answer != "Yeah") && (answer != "Ya") && (answer != "yes") && (answer != "yup") && (answer != "yeah") && (answer != "ya") && (answer != "mhm"));
   
    cout << "I am happy to assist you XD " << endl; 
    tab_auto_completion();
    cout << "Generating the path" << endl;
    reset_is_pressed = false;
    if (inter_to_inter == true) {
        path = findPath(start_intersec, end_intersec);
        isDraw=true;
        drawscreen();
    }
    else {
        path = find_path_to_point_of_interest (start_intersec, end_poi);
        drawscreen();
    }
...
void tab_auto_completion() {
    ...
    cout << "Enter quotation marks if you wanna enable auto-completion function by double pressing tab" << endl;
    cout << "Sample Intersection without auto-completion: Bloor Street & Yonge Street" << endl;
    cout << "Sample Point of Interest with auto-completion: \"RBC Royal Bank\"" << endl;
    cout << "Enter exit/quit/end to close navigation service" << endl;
    cout << "**************************************************************************************" << endl;
    ...
    while (count < 3) {
        char* buf;
        if (count == 1)
            cout << "Tell me where you are (Intersection):" << endl; //starting point
        else if (count == 2)
            cout << "Tell me where you wanna go (Intersection/POI):" << endl; //ending point
        while((buf = readline("")) != NULL) { //Prompt the user for input
            ...
            if(strcmp(buf, "exit") == 0 || strcmp(buf, "quit") == 0 || strcmp(buf, "end") == 0) {
                cout << "Thank you for using Navigation Beta V1.7" << endl;
                free(buf);
                return;
            }
            std::string input_text(buf);
            isPOI = isPOI_or_Intersection(input_text);
            if (count == 1) {
                if (check_intersec_poi_valid(buf) == true && isPOI == false) {
                    break;
                }
                else { 
                    cout << "Oops, the intersection you input is not in our database..." << endl << "Try again now: " << endl;
                }
            }
            else if (count == 2) {
                if (check_intersec_poi_valid(buf) == true) {
                    break;
                }
                else { 
                    cout << "Oops, your input is neither an proper intersection name nor POI in our database..." << endl << "Try again now: " << endl;
                }
            }
        ...
        if (count == 1) {
            if (needs_reverse == false && isPOI == false) 
                start_intersec = find_intersection_id_from_name(no_quo_buf); 
            else if (needs_reverse == true && isPOI == false)
                start_intersec = find_intersection_id_from_name(rev_buf); 
        }
        
        if (count == 2) {
            if (needs_reverse == false && isPOI == false) {
                end_intersec = find_intersection_id_from_name(no_quo_buf); 
                inter_to_inter = true;
            }
            else if (needs_reverse == true && isPOI == false) {
                end_intersec = find_intersection_id_from_name(rev_buf); 
                inter_to_inter = true;
            }
            else if (isPOI == true) {
                end_poi = no_quo_buf;
                inter_to_inter = false;
            }
        }
        free(buf);
        count ++;
...
char* intersection_name_generator(const char* stem_text, int state) {
    ...
    //this int will not changed
    static int count;
    char* ax=(char*)"(unknown)";
    ...
    if (isPOI == true) {    //  POI
        ...
        int ax_len=strlen(ax);
        int text_len = strlen(stem_text);
        //Search through intersections_names until we find a match
        while(count < (int) init_map.POI_names.size()-1) {
            count++;
            if(strncmp(init_map.POI_names[count].c_str(),ax,ax_len)!=0&&strncmp(init_map.POI_names[count].c_str(), stem_text, text_len) == 0) {
                //Must return a duplicate, Readline will handle
                //freeing this string itself.
                return strdup(init_map.POI_names[count].c_str());
            }
        }
        ...
        int text_len = strlen(stem_text);
        //Search through intersections_names until we find a match
        while(count < (int) init_map.intersections_names.size()-1) {
            count++;
            if(strncmp(init_map.intersections_names[count].c_str(),ax,ax_len)!=0&&strncmp(init_map.intersections_names[count].c_str(), stem_text, text_len) == 0) {
                //Must return a duplicate, Readline will handle
                //freeing this string itself.
                return strdup(init_map.intersections_names[count].c_str());
            }
        }
    }
    //No more matches
...
//Zhang
bool check_intersec_poi_valid (char* my_location) {
    
    int word_count = 1;
    int k = 0;
    std::string ampersand;
    std::string myname1, myname2, dbname1, dbname2;
    std::string my_name1, my_name2, my_rd1, my_rd2, my_dir1, my_dir2;  
    //the intersection info stored in database
    std::string db_name1, db_name2, db_rd1, db_rd2, db_dir1, db_dir2;  
    
    // conversion between char* to std::string
    std::string str(my_location); 
    ...
    int string_length = str.length();
     // std::string no_quo_string;
    char* no_quo_char_ptr = (char*) malloc(string_length*sizeof(char));
   
    for (int j=0; j < string_length; j++) {
        if (str[j] != '\"'){
            no_quo_char_ptr[k] = str[j];
            k++;
        }
    } 
    no_quo_char_ptr[k] = '\0';
    
    std::string no_quo_string(no_quo_char_ptr);
    free (no_quo_char_ptr);
    
    //Categorization starts here...
    stringstream mystream (no_quo_string); //change here
    if (word_count == 1) { // POI only
        mystream >> myname1;  
    }
    
    else if (word_count == 2) { // POI only
        mystream >> my_name1 >> my_rd1;
        myname1 = my_name1 + my_rd1;
    }
    // Intersection name only possible starts here
    else if (word_count == 3) { // 
        mystream >> my_name1 >> my_rd1 >> ampersand; 
            myname1 = my_name1 + my_rd1 + ampersand;
    }
    
    else if (word_count == 4) { // POI
        mystream >> my_name1 >> my_rd1 >> ampersand >> my_name2;  
        myname1 = my_name1 + my_rd1 + ampersand + my_name2;
    }
    
    else if (word_count == 5) { // 2 & 2
        mystream >> my_name1 >> my_rd1 >> ampersand >> my_name2 >> my_rd2;  
        if (isPOI) 
            myname1 = my_name1 + my_rd1 + ampersand + my_name2 + my_rd2;
        else { //intersection
            myname1 = my_name1 + my_rd1;
            myname2 = my_name2 + my_rd2;
        }
    }
    
    else if (word_count == 6) { // 2 & 3
        mystream >> my_name1 >> my_rd1 >> ampersand >> my_name2 >> my_rd2 >> my_dir2;
        if (isPOI)
            myname1 = my_name1 + my_rd1 + ampersand + my_name2 + my_rd2 + my_dir2;      
        else {
            if (ampersand == "&") {
                myname1 = my_name1 + my_rd1;
                myname2 = my_name2 + my_rd2 + my_dir2;
            }
            else { //3 & 2
                myname1 = my_name1 + my_rd1 + ampersand;
                myname2 = my_rd2 + my_dir2;
            }
        }
    }
    
    else if (word_count == 7) { // 3 & 3
        mystream >> my_name1 >> my_rd1 >> my_dir1 >> ampersand >> my_name2 >> my_rd2 >> my_dir2;
        if (isPOI)
            myname1 = my_name1 + my_rd1 + my_dir1 + ampersand + my_name2 + my_rd2 + my_dir2;
        else {
        myname1 = my_name1 + my_rd1 + my_dir1;
        myname2 = my_name2 + my_rd2 + my_dir2;
        }
    }
    ...
    if (isPOI == false) {
        ...
        for(auto i=init_map.intersections_names.begin(); i!=init_map.intersections_names.end(); i++)
        {
            stringstream dbstream (*i);
            ...
            dbstream >> dbname1;  
                ...
                dbstream >> db_name1 >> db_rd1;
                dbname1 = db_name1 + db_rd1;
            }
            // Intersection name only possible starts here
            else if (word_count == 3) { // 
                dbstream >> db_name1 >> db_rd1 >> ampersand; 
                    dbname1 = db_name1 + db_rd1 + ampersand;
            }

            else if (word_count == 4) { // POI
                dbstream >> db_name1 >> db_rd1 >> ampersand >> db_name2;  
                dbname1 = db_name1 + db_rd1 + ampersand + db_name2;
            }

            else if (word_count == 5) { // 2 & 2
                dbstream >> db_name1 >> db_rd1 >> ampersand >> db_name2 >> db_rd2;  
                dbname1 = db_name1 + db_rd1;
                dbname2 = db_name2 + db_rd2;
            }
            

            else if (word_count == 6) {       // 2 & 3
                dbstream >> db_name1 >> db_rd1 >> ampersand >> db_name2 >> db_rd2 >> db_dir2;
                if (ampersand == "&") {
                    dbname1 = db_name1 + db_rd1;
                    dbname2 = db_name2 + db_rd2 + db_dir2;
                }
                else { //3 & 2
                    dbname1 = db_name1 + db_rd1 + ampersand;
                    dbname2 = db_rd2 + db_dir2;
                }
            }

            else if (word_count == 7) { // 3 & 3
                dbstream >> db_name1 >> db_rd1 >> db_dir1 >> ampersand >> db_name2 >> db_rd2 >> db_dir2;
                dbname1 = db_name1 + db_rd1 + db_dir1;
                dbname2 = db_name2 + db_rd2 + db_dir2;
            }

    /****************************************************************************************************/
            if ((myname1 == dbname1) && (myname2 == dbname2)) { // normal order                  
                needs_reverse = false;
                no_quo_buf = *i;     
                return true;                                                                    
            }
            else if ((myname1 == dbname2) && (myname2 == dbname1)) { // reverse order
                needs_reverse = true;
                rev_buf = *i;
                return true; 
            }
    /****************************************************************************************************/
        ...
        for(auto j=init_map.POI_names.begin(); j!=init_map.POI_names.end(); j++)
        {
            stringstream dbstream (*j);
            ...
            // Intersection name only possible starts here
                ...
                dbstream >> db_name1 >> db_rd1 >> ampersand >> db_name2 >> db_rd2;  
                dbname1 = db_name1 + db_rd1 + ampersand + db_name2 + db_rd2;
            }

            else if (word_count == 6) { // 2 & 3
                dbstream >> db_name1 >> db_rd1 >> ampersand >> db_name2 >> db_rd2 >> db_dir2;
                dbname1 = db_name1 + db_rd1 + ampersand + db_name2 + db_rd2 + db_dir2;      
            }

            else if (word_count == 7) { // 3 & 3
                dbstream >> db_name1 >> db_rd1 >> db_dir1 >> ampersand >> db_name2 >> db_rd2 >> db_dir2;
                dbname1 = db_name1 + db_rd1 + db_dir1 + ampersand + db_name2 + db_rd2 + db_dir2; 
            }

    /****************************************************************************************************/
            if ((myname1 == dbname1)) { // normal order                  
                no_quo_buf = *j;
                return true;                                                                    
            }
    /****************************************************************************************************/
...
void fun_stuff () {
    ...
    int num;
    cout << "*************************************************" << endl;
    cout << "Please rate your experience of using navigation" << endl << "the path will show up when you finish rating" << endl;
    
    do {
        cout << "*************************************************" << endl;
        cout << "WARNING: Press 0 you will suffer" << endl;
        cout << "Input from 1 (bad) to 5 (great): ";
        cin >> num;
        if (num <= 0  || num >= 6)
            cout << ":/ Invalid input, try again." << endl;
        else if (num == 1)
            cout << "Come on bro, mercy" << endl;
        else if (num == 2)
            cout << "Hmmm...That is not what I want" << endl;
        else if (num == 3)
            cout << "Hey, I'm not that average" << endl; 
        else if (num == 4)
            cout << "I know you like me, why not give me 5 stars?" << endl;
        else if (num == 5)
            break;
    }
    while (num != 5);
    
    cout << "Thank you for your feedback. Your loyalty is our greatest motivation." << endl
            << "Now proceed to the map" << endl;
    
}

bool isPOI_or_Intersection(std::string name) {
    ...
    for (int q=0; q < string_length; q++) {
        if (name[q] != '\"'){
            no_quo_char_ptr[n] = name[q];
            n++;
        }
    } 
    no_quo_char_ptr[n] = '\0';
    ...
    auto POI = init_map.POI.find(no_quo_string); //change here
    if (POI == init_map.POI.end()) 
...
void refresh(void (*drawscreen_ptr) (void)) {
...
event_loop(void (*act_on_mousebutton)(float x, float y, t_event_buttonPressed button_info),
    ...
    create_button("Window","POI ON/OFF",ifdrawPOI); //Zhang
    create_button("POI ON/OFF","Navigation",nav); //Zhang
    create_button("Navigation","Refresh",refresh); //Zhang
...
int init_postscript(const char *fname) {
    ...
    fprintf(gl_state.ps, "/grey95 { .95 setgray } def\n");
    ...
    fprintf(gl_state.ps, "/my_island { 0.65 0.65 0.65 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/firebrick { 0.70 0.13 0.13 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/limegreen { 0.20 0.80 0.20 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/greenyellow { 0.68,1.00,0.18 setrgbcolor} def\n");
    fprintf(gl_state.ps, "/background { 0.17,0.26,0.86 setrgbcolor} def\n");
    fprintf(gl_state.ps, "/lakeontario { 0.68,0.79,0.97 setrgbcolor} def\n");
    fprintf(gl_state.ps, "/my_green { 0.737,0.874,0.60 setrgbcolor} def\n");
    fprintf(gl_state.ps, "/my_forest { 0.768,0.862,0.764 setrgbcolor} def\n");
    fprintf(gl_state.ps, "/my_drain { 0.313,0.474,0.949 setrgbcolor} def\n");
...
/* Creates a init_mapall window at the top of the graphics area for text messages */
static void x11_build_textarea(void) {
    XSetWindowAttributes menu_attributes;
    unsigned long valuemask;
